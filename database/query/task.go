...
func SelectTask(db *gorm.DB, id uint) (task model.Task, err error) {
	err = db.Where("id = ?", id).First(&task).Error
	return
}

// SelectTasks selects a page of tasks for a task from a database
func SelectTasks(db *gorm.DB, storyID uint, limit, offset int) (tasks []model.Task) {
...
func InsertTask(db *gorm.DB, storyID uint, name string) (task model.Task, err error) {
...
func UpdateTask(db *gorm.DB, id uint, name, status string) (task model.Task, err error) {
...
func DeleteTask(db *gorm.DB, id uint) (rows int64, err error) {
	var task model.Task
